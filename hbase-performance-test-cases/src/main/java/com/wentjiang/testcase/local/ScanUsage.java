...
//        getData(tableName,"row_key:field1:1");
    ...
    private static void putBatchData() {
        for (int i = 0; i < 100; i++) {
            client.putData(tableName,"row_key:field1:" + i,familyName,field1, String.valueOf(i), Durability.SYNC_WAL);
        }

        for (int i = 0; i < 100; i++) {
            client.putData(tableName,"row_key:field2:" + i,familyName,field2, String.valueOf(i), Durability.SYNC_WAL);
        }

        for (int i = 0; i < 100; i++) {
            client.putData(tableName,"row_key:field3:" + i,familyName,field3, String.valueOf(i), Durability.SYNC_WAL);
        }
    }
